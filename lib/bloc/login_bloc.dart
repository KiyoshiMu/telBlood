import 'dart:async';
import 'package:telblood/classes/validators.dart';
...
class LoginBloc with Validators {
  ...
  StreamController<String>.broadcast();
  Sink<String> get passwordChanged =>
    _passwordController.sink;
  Stream<String> get password =>
    _passwordController.stream.transform(validatePwd);
  ...
  final StreamController<String> _loginOrCreateController
    = StreamController<String>();
  Sink<String> get loginOrCreateChanged =>
    _loginOrCreateController.sink;
  Stream<String> get loginOrCreate =>
    _loginOrCreateController.stream;
...
_startListenersIfEmailPasswordAreValid();
  ...
  Future<String> _logIn() async {
    ...
    if(_emailValid && _passwordValid) {
      await
      authenticationApi.signInWithEmails(email:
        _email, pwd: _password).then((user) {
  ...
  Future<String> _createAccount() async {
    ...
    if(_emailValid && _passwordValid) {
    await
    authenticationApi.createUser(email:
      _email, pwd: _password).then((user) {
